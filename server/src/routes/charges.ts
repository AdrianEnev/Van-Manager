...
export async function registerChargeRoutes(app: FastifyInstance) {
    ...
    // User: get overdue charges
    app.get('/api/my/charges/overdue', async (req: FastifyRequest, reply: FastifyReply) => {
        const ctx = await requireAuth(app, req, reply);
    ...
    // User: get upcoming charges (due within X days)
    app.get('/api/my/charges/upcoming', async (req: FastifyRequest, reply: FastifyReply) => {
        const ctx = await requireAuth(app, req, reply);
        const qSchema = z.object({ days: z.coerce.number().min(1).max(30).default(7) });
        const parsed = qSchema.safeParse((req as any).query);
        if (!parsed.success) return reply.code(400).send({ error: parsed.error.flatten() });
