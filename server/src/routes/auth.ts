...
export async function registerAuthRoutes(app: FastifyInstance) {
    ...
    app.post('/auth/register', async (req: FastifyRequest, reply: FastifyReply) => {
        ...
        return reply.send({
            user: { id: user._id, email: user.email, name: user.name, role: user.role, isTransactionAllowed: !!user.isTransactionAllowed },
    ...
    app.get('/auth/me', async (req: FastifyRequest, reply: FastifyReply) => {
        try {
            ...
            return reply.send({ id: user._id.toString(), email: user.email, name: user.name, role: user.role, isTransactionAllowed: !!user.isTransactionAllowed });
        } catch (e) {
            return reply.code(401).send({ error: 'Invalid token' });
        }
    });
