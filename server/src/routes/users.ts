...
export async function registerUserRoutes(app: FastifyInstance) {
    ...
    app.post('/api/users/login/google', async (req: FastifyRequest, reply: FastifyReply) => {
        ...
        try {
            ...
            // Generate standard tokens for client usage
            ...
            return reply.send({ message: 'Login successful', isNewUser, tokens: { accessToken, refreshToken }, user: { id: user!._id, email: user!.email, name: user!.name, role: user!.role, isTransactionAllowed: !!user!.isTransactionAllowed } });
        } catch (e) {
