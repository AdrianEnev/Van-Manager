...
import { adminCreatePlan, adminListCharges, adminListUsers, adminListUserVehicles, adminListPlans, adminMarkChargePaid, adminUpdatePlan, adminDeletePlan, type AdminUser, type Charge, type VehicleAssignment, type Plan } from '../../../lib/api';
...
export default function AdminChargesPage() {
    ...
    async function handleUpdatePlan(e: React.FormEvent<HTMLFormElement>) {
        ...
        try {
            await adminUpdatePlan(editingPlan.id, { amount, currency, active });
            setSuccess('Plan updated successfully.');
            setEditingPlan(null);
            await refreshPlans(selectedUser);
        } catch (err: any) {
            setError(err?.message || 'Update plan failed');
        }
    }
    ...
    async function handleDeletePlan(planId: string) {
        try {
            ...
            setError(err?.message || 'Delete plan failed');
            setDeletingPlanId(null);
        }
    }
    ...
    return (
        ...
        >
                            ...
                            <div key={p.id} className="rounded-2xl border px-4 py-3">
                                <div className="flex justify-between items-start gap-3">
                                    ...
                                    <div className="flex flex-col gap-2 shrink-0">
                                        <Button size="sm" variant="outline" onClick={() => setEditingPlan(p)}>Edit</Button>
                                        <Button size="sm" variant="ghost" className="text-red-600 hover:text-red-700 hover:bg-red-50" onClick={() => setDeletingPlanId(p.id)}>Delete</Button>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div className="bg-white rounded-lg shadow-xl max-w-md w-full mx-4" onClick={(e) => e.stopPropagation()}>
                        ...
                        <form onSubmit={handleUpdatePlan} className="p-6 space-y-4">
                            ...
                            <div>
                                ...
                                <select name="active" defaultValue={String(editingPlan.active)} className="w-full rounded border px-3 py-2">
                            ...
                            <div className="text-sm text-gray-600 bg-gray-50 p-3 rounded">
                                ...
                                <p>Frequency and starting date cannot be changed. To change frequency, deactivate this plan and create a new one.</p>
                            </div>
            ...
            {deletingPlanId && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setDeletingPlanId(null)}>
                    <div className="bg-white rounded-lg shadow-xl max-w-md w-full mx-4" onClick={(e) => e.stopPropagation()}>
                        <div className="border-b px-6 py-4">
                            <h2 className="text-lg font-semibold text-red-700">Delete Plan</h2>
                        </div>
                        <div className="p-6">
                            <p className="text-gray-700 mb-4">Are you sure you want to delete this plan? This action cannot be undone.</p>
                            <p className="text-sm text-gray-600 bg-yellow-50 border border-yellow-200 p-3 rounded">
                                <strong>Note:</strong> Deleting this plan will not affect charges that have already been created. Those charges will remain in the system.
                            </p>
                            <div className="flex gap-2 justify-end mt-6">
                                <Button type="button" variant="outline" onClick={() => setDeletingPlanId(null)}>Cancel</Button>
                                <Button type="button" className="bg-red-600 hover:bg-red-700" onClick={() => handleDeletePlan(deletingPlanId)}>Delete Plan</Button>
                            </div>
                        </div>
                    </div>
                </div>
