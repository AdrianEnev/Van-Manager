//export const API_URL = process.env.NEXT_PUBLIC_API_URL || 'https://van-manager.fly.dev';
...
export async function initiatePayment(chargeId: string): Promise<{ sessionId: string; url: string }> {
    return apiFetchAuto<{ sessionId: string; url: string }>(`/api/payments/checkout`, {
        method: 'POST',
...
export async function submitContactForm(payload: { name: string; email: string; phone?: string; message: string }): Promise<{ success: boolean; message: string }> {
    const headers = new Headers();
    headers.set('Content-Type', 'application/json');
